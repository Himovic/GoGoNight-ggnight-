		...
		$('input[name="pref"]:checked').each(function() {
			PreferenceAlimentaire.push(this.value);
                ...
                if(password.length <9){
                    getMsg("Votre mot de passe doit étre supérieur a 8 caractéres");
                }else if(phone.length < 11 || phone[0] !== "0"){
                    getMsg("Le numéro de téléphone doit étre supérieur a 10 caractéres et il commence par zéro(0)");
                }else{
                    $.ajax({
